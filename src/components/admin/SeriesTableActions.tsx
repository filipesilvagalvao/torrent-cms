...
export default function SeriesTableActions({
  id,
  initialPublished,
}: Props) {
  ...
  async function patch(action: "publish" | "feature") {
    ...
    try {
      ...
      const json = (await res.json()) as { published?: boolean; featured?: boolean; error?: string };
  ...
  async function deleteItem() {
    if (!confirm("Excluir a série definitivamente? Temporadas e episódios também serão removidos.")) return;
    setBusy("delete");
    setFeedback(null);
    try {
    ...
    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem" }}>
      ...
      >
        {busy === "publish" ? (
          ...
          <FontAwesomeIcon icon={initialPublished ? faEyeSlash : faEye} />
        )}
      </button>
      <button
        type="button"
